...
SPECIAL_TOKENS = {
    ...
    "eos_token": "[EOS]",
    "pad_token": "[PAD]",
    "additional_special_tokens": ["[SYS]", "[USR]", "[KG]","[SUB]", "[PRED]","[OBJ]","[TRIPLE]", "[SEP]"],
}
...
class BaseDataset(Dataset):
    def __init__(self, args, tokenizer, name, split_type, labels=True, labels_file=None):
        ...
        self.sys_token, self.usr_token, self.kg, self.sub_token, self.pred_token, self.obj_token, self.triple_token, self.sep = self.tokenizer.convert_tokens_to_ids(self.SPECIAL_TOKENS["additional_special_tokens"])
    ...
    def build_input_from_segments(self, knowledge, history, response, example, with_eos=True):
        ...
        instance["entity_ids"] = [1]*len(instance["input_ids"]) if len(example["entity_ids"])==0 else example["entity_ids"][:len(instance["lm_labels"])]
        instance["triple_ids"] = [1]*len(instance["input_ids"]) if len(example["triple_ids"])==0 else example["triple_ids"][:len(instance["lm_labels"])]
